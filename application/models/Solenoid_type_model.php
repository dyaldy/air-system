...
/**
 * Solenoid_type_model
 * Manages solenoid types and their images.
 */
class Solenoid_type_model extends CI_Model
{
    private $table = 'as_solenoid_types';
    ...
    /**
     * Check if a solenoid type is being used by any solenoid records.
     *
    ...
    {
        $this->db->where('type', strtoupper($type));
        return $this->db->count_all_results('as_solenoid') > 0;
    }

    /**
     * Get count of solenoid records using this type.
     *
     * @param string $type The type to check
     * @return int Number of solenoid records using this type
     */
    public function getUsageCount(string $type): int
    {
        $this->db->where('type', strtoupper($type));
        return $this->db->count_all_results('as_solenoid');
    }
}
